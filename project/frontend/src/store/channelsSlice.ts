...
>(
    ...
    async ({ channelId, query, offset, limit }, thunkAPI) => {
        try {
            const response = await fetchData<{data: Message[]}>(
                `${import.meta.env.VITE_API_URL}/messages/search?channel=${channelId}&query=${query}&offset=${offset}&limit=${limit}`
            );
            return {
...
export const {utilizeSearchedMessages} = channelsSlice.actions;
